...
namespace Open_Miracle
{
    public class DBConnection
    {
        ...
        protected string database = (ConfigurationManager.AppSettings["ApplicationPath"] == null || ConfigurationManager.AppSettings["ApplicationPath"].ToString() == string.Empty) ? null : ConfigurationManager.AppSettings["ApplicationPath"].ToString();
        ...
        public DBConnection()
        {
            ...
            if (serverName != null)
            {
                if (isSqlServer != null)
                {
                    ...
                    {
                        sqlcon = new SqlConnection(@"Data Source=" + serverName + ";database=" + database + ";Integrated Security=True;Connect Timeout=120");
                    }
                    else
                    {
                        sqlcon = new SqlConnection(@"Data Source=" + serverName + ";database=" + database + ";user id='" + userId + "';password='" + password + "'; Connect Timeout=120");
                    }
                }
                else
                {
                    if (userId == null || password == null)
                    {
                        sqlcon = new SqlConnection(@"Data Source=" + serverName + ";database=" + database + ";Integrated Security=True;Connect Timeout=120;User Instance=True");
                    }
                    else
                    {
                        sqlcon = new SqlConnection(@"Data Source=" + serverName + ";database=" + database + ";user id='" + userId + "';password='" + password + "'; Connect Timeout=120; User Instance=False");
                    }
                }
                try
                {
