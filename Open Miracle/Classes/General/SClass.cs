...
namespace Open_Miracle
{
    ...
    class SClass
    {
        ...
        internal bool CheckMsSqlConnection(string serverName, string userId, string password, string database)
        {
            ...
            if (serverName != null)
            {
                ...
                {
                    sqlcon = new SqlConnection(@"Data Source=" + serverName + ";database=" + database + ";Integrated Security=True;Connect Timeout=30;User Instance=True");
                }
                else
                {
                    sqlcon = new SqlConnection(@"Data Source=" + serverName + ";database=" + database + ";user id='" + userId + "';password='" + password + "'; Connect Timeout=30; User Instance=False");
                }
                try
                {
                    sqlcon.Open();
                ...
                catch (SqlException ex)
                {
                    if (ex.Number == 18493)
                    {
                        if (userId == null || password == null)
                        {
                            sqlcon = new SqlConnection(@"Data Source=" + serverName + ";database=" + database + ";Integrated Security=True;Connect Timeout=30");
                        }
                        else
                        {
                            sqlcon = new SqlConnection(@"Data Source=" + serverName + ";database=" + database + ";user id='" + userId + "';password='" + password + "'; Connect Timeout=30");
                        }
                        try
                        {
                            sqlcon.Open();
        ...
        internal DataTable GetOmPath(string serverName, string userId, string password)
        {
            ...
            if (serverName != null)
            {
                ...
                {
                    sqlcon.Open();
                    SqlDataAdapter sqlDa = new SqlDataAdapter("SELECT name, cast(replace(physical_name, '\\Data\\DBOpenMiracle.mdf', '') as varchar(max)) AS [location] FROM sys.master_files WHERE physical_name like '%'+'\\Data\\DBOpenMiracle.mdf'", sqlcon);
